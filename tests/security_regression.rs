...
fn test_authenticated_consensus_flood_triggers_isolation() {
    ...
    let disconnected = events.iter().any(|e| matches!(&e.event, TelemetryEvent::LinkRemoved { source, target }
        if (source == id_a && target == id_d) || (source == id_d && target == id_a)));
        
    assert!(disconnected, "A should have disconnected from D due to consensus votes");
}
...
fn test_quarantined_node_ignored_next_step() {
    ...
    assert!(!a_emitted_step_1, "A should not emit signal if it ignored B");
}
