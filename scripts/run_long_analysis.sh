...
ARTIFACT_ROOT="target/long_analysis"
rm -rf "$ARTIFACT_ROOT"
...
# --- Run with Hybrid Mutation ---
HYBRID_RUN_DIR="$ARTIFACT_ROOT/hybrid"
mkdir -p "$HYBRID_RUN_DIR"
echo "Running with Hybrid mutation strategy..."
cargo run --bin adversarial_loop -- \
    --state "$HYBRID_RUN_DIR/harness_state.json" \
    --generations 20 \
    --artifact-dir "$HYBRID_RUN_DIR" \
    --mutation-strategy Targeted \
    --exploration-generations 5 \
    --max-generations 1000 \
    --seed "baseline=docs/examples/baseline-growth.yaml" \
    --seed "intense=docs/examples/intense-defense.yaml"

echo "Long analysis runs complete."
