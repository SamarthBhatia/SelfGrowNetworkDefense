...
- `baseline_summary.json` / `intense_summary.json` — run-level statistics and annotations.
...
3. Open `intense_summary.json` → examine the `annotations` block for breach status and fitness.
4. Open `intense_outcome.json` → note recommended mutation and backlog growth.
5. Load `intense_vega.json` in a Vega viewer → visualise threat vs. replication over time.
